      ...
      <div class="guess">
        ...
        <div class="input-container">
          ...
          <el-row type="flex" justify="center" align="middle" :gutter="20" class="button-row">
            <el-col :span="isMobile ? 8 : 4" class="button-col">
              <el-button type="primary" class="action-button" :disabled="this.mygameover"
                         @click="Guess()">
                {{ this.gameover ? '已结束' : '确定' }}
              </el-button>
            </el-col>
            <el-col v-if="this.host" :span="isMobile ? 8 : 4" class="button-col">
        ...
        <el-row>
          ...
          <div v-if="isMobile" class="pokemon-cards mobile-cards">
            <div v-for="(item, index) in (()=>displayedItems(this.tableData))()" :key="index" class="pokemon-card">
          ...
          <!-- 移动端卡片垂直布局 -->
          <div v-if="isMobile" class="pokemon-cards mobile-cards">
            <div v-for="(item, index) in (()=>displayedItems(this.tableData2))()" :key="index" class="pokemon-card">
              <div class="card-header">
                <div class="pokemon-image">
                  <el-image style="width: 50px; height: 50px" :src="item.imgUrl" fit="contain"></el-image>
                </div>
...
export default {
  data() {
    return {
      ...
      maxguess: 10,
      settings: {
        selectedGens: this.$route.params.settings.selectedGens,
        hardid: this.$route.params.settings.hardid,
        maxguess: this.$route.params.settings.maxGuess,
        battleOpen: this.$route.params.settings.battleOpen,
        shapeOpen: this.$route.params.settings.shapeOpen,
        catchOpen: this.$route.params.settings.catchOpen,
        cheatOpen: this.$route.params.settings.cheatOpen,
        showGenArrow: this.$route.params.settings.showGenArrow,
        reverseDisplay: this.$route.params.settings.reverseDisplay, // 控制卡片展示顺序
      },
    ...
    async Guess() {
      try {
        ...
        if (data == "guess name error") {
          ...
          if (this.times === this.settings.maxguess && this.times2 === this.maxguess) {
            this.gameover = true
          }
  ...
  mounted() {
    ...
    this.socket.on("answer_result", (data) => {
      ...
      if (this.times2 === this.settings.maxguess && this.times === this.settings.maxguess) {
        this.gameover = true
        this.ReplayAnswer()
      }
    })
...
.pokemon-image .el-icon-picture-outline {
  display: flex;
  justify-content: center;
  align-items: center;
  width: 100%;
		...
		<el-main>
			<div class="guess">
				...
				<div class="input-container">
					...
					<el-row type="flex" justify="center" align="middle" class="input-row">
						<div style="width: 50%;">
							<el-progress :percentage="this.countdown/(this.settings.maxguess*12)*100" :format="countdownFormat"></el-progress>
						</div>
					</el-row>
					<!-- 增加按钮与输入框之间的间距 -->
								...
								<div class="p2-desktop-card-content">
									...
									<div class="p2-desktop-section">
										...
										<div class="section-content">
											...
											<el-tag v-if="settings.battleOpen" size="small" :type="item.speed.col" class="info-tag" effect="dark"></el-tag>
											</div>
										</div>
									</div>
									...
									<div class="p2-desktop-section">
										...
										<div class="section-content">
											<div>
												<el-tag v-if="item.evo.key != null" size="small" :type="item.evo.col" class="info-tag" effect="dark"></el-tag>
